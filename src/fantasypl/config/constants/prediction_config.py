"""Constants required during gameweekly predictions."""
...
"""
The scaling factor for the team-level model predictions to better
match the true spread of the stats.
"""
...
"""
From a set of about 1500 top managers from top FPL leagues of GW 1-3
in 2024-25 season, this ratio was calculated from taking the GW 1
subs, and seeing how many times they have featured in the following
two weeks, plus any auto-subs that have happened.
"""

WEIGHTS_DECAYS_BASE: list[float] = [0.865, 0.585]
"""
0.48 is the linear regression slope between one week's top scorers'
next week score and the max score of next week for 2023-24. Then
computing the total information kept by each week, and then
normalizing to unit sum and then dividing by the first week numbers,
we find the decay score.
"""

TRANSFER_HIT_PENALTY_PERCENTILE: float = 77.03
"""
FPL transfer hit penalty is 4, which computes to 77.03 percentile of
all FPL gameweek scores from the last 3 seasons for players with
minutes > 0.
"""

TRANSFER_GAIN_MINIMUM: float = 0.0404 * (1 + sum(WEIGHTS_DECAYS_BASE))
"""
Standard deviation for the current xpoints model is 0.32, which equates
to a standard error of 0.0146 with predicted gameweek points for
